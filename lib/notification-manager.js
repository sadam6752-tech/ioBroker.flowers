...
class NotificationManager {
     ...
     *
     * @returns {boolean} true if current time is within night mode hours
     */
    _isNightMode() {
        ...
        const now = new Date();
        const [startH, startM] = (cfg.nightModeStart || '22:00').split(':').map(Number);
     ...
     *
     * @param {string} key - cooldown key in format "plantName:alertType"
    ...
    async send(text, cooldownKey, force = false) {
        ...
        if (!force && this.sentToday >= maxPerDay) {
            this.adapter.log.debug(`NotificationManager: daily limit reached (${maxPerDay}), skipping`);
            return false;
        }

        if (!force && this._isNightMode()) {
            this.adapter.log.debug('NotificationManager: night mode active, skipping');
            return false;
        }

        if (cooldownKey && this._inCooldown(cooldownKey)) {
            this.adapter.log.debug(`NotificationManager: cooldown active for ${cooldownKey}, skipping`);
            return false;
        }
    ...
    /**
     * @param {Array} plantStates - array of { plant, humidity, temperature, battery }
     * @param {string} headerKey - message key for report header
     * @returns {Promise<void>}
     */
