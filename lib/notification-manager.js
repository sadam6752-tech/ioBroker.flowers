...
class NotificationManager {
  ...
  constructor(adapter) {
    ...
    this.cooldowns = {};
    this.COOLDOWN_MS = 60 * 60 * 1000; // 1 hour between same alerts
  }
    ...
    try {
      if (users.length > 0) {
        for (const user of users) {
          await this.adapter.sendToAsync(`telegram.${instance}`, "send", {
            user,
            text,
          });
        }
      ...
      this.adapter.log.debug(
        `NotificationManager: sent (${this.sentToday}/${maxPerDay}): ${text.substring(0, 60)}`,
      );
      return true;
    } catch (err) {
      this.adapter.log.error(
        `NotificationManager: sendTo error: ${err.message}`,
      );
      return false;
    }
  }
