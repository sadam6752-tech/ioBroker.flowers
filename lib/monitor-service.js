...
class MonitorService {
  ...
  async _startWatering(plant) {
    if (!plant.sensorWatering) return;
    ...
    try {
      await this.adapter.setForeignStateAsync(plant.sensorWatering, { val: true, ack: false });
      ...
      this.adapter.setTimeout(async () => {
        try {
          await this.adapter.setForeignStateAsync(plant.sensorWatering, { val: false, ack: false });
          this.adapter.log.info(`MonitorService: watering stopped for "${plant.name}"`);
        } catch (err) {
          this.adapter.log.warn(`MonitorService: failed to stop watering for "${plant.name}": ${err.message}`);
        }
