...
class MonitorService {
   ...
   */
  constructor(adapter, notificationManager) {
  ...
  async onStateChange(id, state) {
    ...
    this.sensorValues[id] = { val: state.val, ts: state.ts || Date.now() };
      ...
      if (entry != null) {
        ...
        if (val < thresholds.humidityMin) {
          await this.notif.send(
            `💧 ${label}: soil too DRY — ${val}% (min ${thresholds.humidityMin}%). Time to water!`,
            `${plant.name}:humidity_low`,
          );
        } else if (val > thresholds.humidityMax) {
          await this.notif.send(
            `💦 ${label}: soil too WET — ${val}% (max ${thresholds.humidityMax}%). Check drainage!`,
    ...
    if (plant.sensorTemperature) {
      ...
      if (entry != null) {
        ...
        if (val < thresholds.temperatureMin) {
          await this.notif.send(
            `🥶 ${label}: too COLD — ${val}°C (min ${thresholds.temperatureMin}°C)`,
            `${plant.name}:temp_low`,
          );
        } else if (val > thresholds.temperatureMax) {
          await this.notif.send(
            `🥵 ${label}: too HOT — ${val}°C (max ${thresholds.temperatureMax}°C)`,
    ...
    if (plant.sensorBattery) {
      ...
      if (entry != null) {
        ...
        if (val < thresholds.batteryMin) {
          await this.notif.send(
            `🔋 ${label}: battery LOW — ${val}% (min ${thresholds.batteryMin}%). Replace battery!`,
  ...
  async _checkOffline(plant) {
    ...
    if (diffHours >= offlineHours) {
      const label = `${plant.name}${plant.location ? ` (${plant.location})` : ""}`;
      await this.notif.send(
        `📡 ${label}: sensor OFFLINE for ${Math.round(diffHours)}h. Check connection!`,
        `${plant.name}:offline`,
      );
    }
  }
