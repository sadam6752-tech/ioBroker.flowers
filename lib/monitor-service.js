...
class MonitorService {
    ...
    _getThresholds(plant) {
        ...
        const profile = customProfile || profiles[profileName] || profiles['Custom'];
    ...
    async subscribeAll() {
        ...
        for (const plant of plants) {
            ...
            for (const attr of ['sensorHumidity', 'sensorTemperature', 'sensorBattery']) {
    ...
    async onStateChange(id, state) {
        ...
        for (const plant of plants) {
            ...
            if (plant.sensorHumidity === id || plant.sensorTemperature === id || plant.sensorBattery === id) {
                this.lastSeen[plant.name] = Date.now();
    ...
    async _checkPlant(plant) {
        ...
        if (plant.sensorHumidity) {
            ...
            if (entry != null) {
                ...
                await this._updateState(`plants.${this._safeName(plant.name)}.humidity`, val, '%');
            }
        }
        ...
        if (plant.sensorTemperature) {
            ...
            if (entry != null) {
                ...
                await this._updateState(`plants.${this._safeName(plant.name)}.temperature`, val, '°C');
            }
        }
        ...
        if (plant.sensorBattery) {
            ...
            if (entry != null) {
                ...
                await this._updateState(`plants.${this._safeName(plant.name)}.battery`, val, '%');
            }
        }
    }
    ...
    async _checkOffline(plant) {
        ...
        if (diffHours >= offlineHours) {
            const label = `${plant.name}${plant.location ? ` (${plant.location})` : ''}`;
            await this.notif.send(msg('offline', this.lang, label, Math.round(diffHours)), `${plant.name}:offline`);
        }
    }
    ...
    async _startWatering(plant) {
        ...
        if (this._wateringActive[plant.name]) {
            this.adapter.log.debug(`MonitorService: watering already active for "${plant.name}", skipping`);
            return;
        }
        ...
        try {
            ...
            this.adapter.log.info(`MonitorService: watering started for "${plant.name}" (${durationMin} min)`);
            ...
            this.adapter.setTimeout(async () => {
                try {
                    ...
                    this.adapter.log.info(`MonitorService: watering stopped for "${plant.name}"`);
                } catch (err) {
                    this.adapter.log.warn(
            ...
            this.adapter.log.warn(`MonitorService: failed to start watering for "${plant.name}": ${err.message}`);
            this._wateringActive[plant.name] = false;
        }
    }
    ...
    async _updateState(id, val, unit) {
        try {
            // Ensure parent objects exist: plants (folder) → plants.<name> (channel)
            ...
            this.adapter.log.warn(`MonitorService: failed to update state ${id}: ${err.message}`);
        }
    }
    ...
    getPlantStates() {
        ...
        return plants
            ...
            .map(plant => ({
                plant,
                humidity: plant.sensorHumidity ? (this.sensorValues[plant.sensorHumidity]?.val ?? null) : null,
                temperature: plant.sensorTemperature ? (this.sensorValues[plant.sensorTemperature]?.val ?? null) : null,
                battery: plant.sensorBattery ? (this.sensorValues[plant.sensorBattery]?.val ?? null) : null,
            }));
    }
}
